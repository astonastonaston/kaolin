...
def compute_losses(
    ...
    # --- optional material regularization ---
    material_reg_tensors: Optional[Iterable[Tuple[torch.Tensor, torch.Tensor]]] = None,
    ...
    # ---- material reg (added into le for logging consistency) ----
    if material_reg_tensors is not None and material_reg_coeff and material_reg_coeff > 0.0:
        reg = torch.tensor(0.0, device=device, dtype=dtype)
        for p, p0 in material_reg_tensors:
            reg = reg + torch.mean((p - p0) ** 2)
        le = le + material_reg_coeff * reg
