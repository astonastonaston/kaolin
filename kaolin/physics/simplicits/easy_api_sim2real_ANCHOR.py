...
try:
    from losses_sim2real_UPDATED import compute_losses  # local patched losses
except Exception:
...
class SimplicitsObject:
    ...
    def create_trained(pts, yms, prs, rhos, appx_vol,
                       ...
                       # --- optional: static anchor constraints (to match eval) ---
                       sim2real_anchor_enable=True,
                       sim2real_anchor_axis=0,          # 0=x,1=y,2=z
                       sim2real_anchor_k=50,            # points to anchor per side
                       sim2real_anchor_penalty=1e6,
                       sim2real_anchor_ids_min=None,    # optional (K,) indices into qp points
                       sim2real_anchor_ids_max=None,    # optional (K,) indices into qp points
        ...
        if (training_ls_coeff is not None) and (training_ls_coeff > 0.0) and (sim2real_real_frames is not None) and (sim2real_train_range is not None) and (sim2real_tool_t is not None):
            ...
            def _rollout_fn(current_model: torch.nn.Module, t_idx: int) -> torch.Tensor:
                ...
                # Re-apply static anchors (keeps train/eval constraints consistent)
                if sim2real_anchor_enable and (anchor_p0_min is not None) and (anchor_p0_max is not None):
                    det_scene.update_object_boundary_pinned_x("anchor_min", anchor_p0_min)
                    det_scene.update_object_boundary_pinned_x("anchor_max", anchor_p0_max)
