...
try:
    from kaolin.physics.simplicits.losses_sim2real_UPDATED_anchor_boundary import compute_losses  # local patched losses
except Exception:
...
class SimplicitsObject:
    ...
    def create_trained(pts, yms, prs, rhos, appx_vol,
        ...
        if (training_ls_coeff is not None) and (training_ls_coeff > 0.0) and (sim2real_real_frames is not None) and (sim2real_train_range is not None) and (sim2real_tool_t is not None):
            ...
            # # -----------------------------
            # # Anchor constraint (static): anchor points near the AABB boundary faces
            # # This matches the simple "anchor boundary points" logic you showed in the notebook.
            # # -----------------------------
            ...
            #     else:
            #         # User-provided indices into qp points
            ...
            #     # Exclude grasp points from anchors (avoid conflicting constraints)
            #     anchor_mask_gpu[grasp_ids] = False

            #     # Kaolin boundary selector expects a CPU boolean mask of length M
            ...
            # # pick grasp ids if needed (closest qp points to tool at t0)
            ...
            # else:
            #     grasp_ids = sim2real_grasp_ids.to(device=device)

            # grasp_mask_cpu = torch.zeros((internal_rest.shape[0],), dtype=torch.bool).cpu()
            ...
            #     # scene expects a boolean mask of length M (qp count)
            ...
            # # Initialize boundary once (we'll update pinned_x each frame)
            # det_scene.set_object_boundary_condition(
            #     det_obj_idx,
            #     name="grasp",
            #     fcn=_grasp_fcn,
        ...
        for i in range(training_num_steps):
            ...
            le, lo, ls = compute_losses(model, 
                                    ...
                                    sim2real_num_pts_sim=sim2real_num_pts_sim, # number of pts to compute chamfer loss
            ...
            if i % training_log_every == 0:
                logger.info(
                    f'Training step: {i}, le: {le.item()}, lo: {lo.item()}, ls: {ls.item()}')
