...
from kaolin.physics.simplicits.losses_sim2real_UPDATED_anchor_boundary_fastckpt_v3 import compute_losses  # local patched losses
...
class SimplicitsObject:
    ...
    def create_trained(pts, yms, prs, rhos, appx_vol,
                       ...
                       # --- optional: anchor constraint based on boundary (AABB faces) ---
                       sim2real_anchor_enable=True,
                       sim2real_anchor_eps_ratio=0.03,   # fraction of bbox extent per axis
                       sim2real_anchor_penalty=1e7,
                       sim2real_anchor_ids=None,         # optional (K,) qp indices (CPU/GPU ok)
                       ...
                       training_save_config=True,
                       training_return_logs=False,
                       # --- checkpoints / resume ---
                       training_ckpt_dir=None,
                       training_ckpt_every=500,
                       # aliases (older notebooks)
                       training_save_every=None,
                       training_eval_every=None,
                       training_keep_last=5,
                       training_resume=False,
                       training_resume_from=None,
                       # --- speed knobs for Sim2Real term ---
                       sim2real_every=10,
                       sim2real_warmup_steps=0):
        ...
        # -----------------------------
        if training_save_every is not None:
            training_ckpt_every = training_save_every
        # training_eval_every is used for periodic *evaluation* (no grad), not for logging
        eval_every = training_eval_every if training_eval_every is not None else None

        # -----------------------------
        # Optional: build a tool-driven rollout function for Sim2Real
        # -----------------------------
        sim2real_rollout_fn = None
        if (training_ls_coeff is not None) and (training_ls_coeff > 0.0) and (sim2real_real_frames is not None) and (sim2real_train_range is not None) and (sim2real_tool_t is not None):
            # We run the physics solver with *detached* weights (so Z_t is treated as constant),
            # then compute a differentiable LBS deformation using the *current* model weights.
            #
            # This gives a stable first-order approximation of d(Chamfer)/dW without
            # backpropagating through Newton's method.
            tool_t = sim2real_tool_t.to(device=device, dtype=pts.dtype)
            tool_R = None
            if sim2real_tool_R is not None:
                tool_R = sim2real_tool_R.to(device=device, dtype=pts.dtype)

            # Build a scene once; we will reset it per rollout call.
            class _DetachModel(torch.nn.Module):
            ...
            det_model = _DetachModel(model)
            det_obj = SimplicitsObject(pts, yms, prs, rhos, appx_vol, det_model)

            det_scene = SimplicitsScene()
            det_scene.max_newton_steps = int(sim2real_scene_newton_steps)
            det_scene.timestep = float(sim2real_scene_timestep)
            det_scene.direct_solve = bool(sim2real_scene_direct_solve)

            det_obj_idx = det_scene.add_object(det_obj, num_qp=int(sim2real_scene_num_qp))
            det_scene.set_scene_gravity(acc_gravity=torch.tensor([0.0, 0.0, 0.0], device=device, dtype=pts.dtype))
            det_scene.reset_scene()

            internal_rest = det_scene.get_object_deformed_pts(det_obj_idx)  # (M,3) qp points in rest


            # -----------------------------
            # 1) Pick grasp ids first (needed to exclude from anchors)
            # -----------------------------
            if sim2real_grasp_ids is None:
                t0_tmp = tool_t[0]
                d2 = torch.sum((internal_rest - t0_tmp[None, :])**2, dim=1)
                grasp_ids = torch.topk(d2, k=int(sim2real_grasp_k), largest=False).indices
            else:
                grasp_ids = sim2real_grasp_ids.to(device=device).long()

            # sanitize (important!)
            M = internal_rest.shape[0]
            grasp_ids = grasp_ids[(grasp_ids >= 0) & (grasp_ids < M)]
            if grasp_ids.numel() == 0:
                raise ValueError("sim2real_grasp_ids became empty after sanitization. Check indices / num_qp.")
            ...
            t0 = tool_t[0].clone()
            R0 = tool_R[0].clone() if tool_R is not None else None

            def _grasp_fcn(_deformed_pts):
                return grasp_mask_cpu

            def _targets_translation(t_idx: int) -> torch.Tensor:
                return p0 + (tool_t[t_idx] - t0)[None, :]

            def _targets_with_rot(t_idx: int) -> torch.Tensor:
                Rt = tool_R[t_idx]
                tt = tool_t[t_idx]
                return tt[None, :] + (Rt @ (R0.transpose(0, 1) @ (p0 - t0[None, :]).T)).T

            # def _lbs_deform_points(rest_pts: torch.Tensor, weights: torch.Tensor, transforms_3x4: torch.Tensor) -> torch.Tensor:
            #     # rest_pts: (M,3), weights: (M,H), transforms_3x4: (H,3,4)
            #     M = rest_pts.shape[0]
            #     ones = torch.ones((M, 1), device=rest_pts.device, dtype=rest_pts.dtype)
            #     hom = torch.cat([rest_pts, ones], dim=1)  # (M,4)
            #     # (M,H,3)
            #     deltas = torch.einsum('md,hnd->mhn', hom, transforms_3x4.transpose(1, 2))
            #     disp = (deltas * weights.unsqueeze(-1)).sum(dim=1)
            #     return rest_pts + disp

            def _lbs_deform_points(rest_pts: torch.Tensor,
                       weights: torch.Tensor,
                       transforms: torch.Tensor) -> torch.Tensor:
                """
                rest_pts:   (M,3)
                weights:    (M,H)
                transforms: (H,3,4)  [relative transforms: (T - I) in 3x4 form]
                        or (H,4,4) [relative in 4x4 form]
                returns:    (M,3)
                """
                # Accept either (H,4,4) or (H,3,4)
                if transforms.shape[-2:] == (4, 4):
                    transforms_3x4 = transforms[:, :3, :]          # (H,3,4)
                else:
                    transforms_3x4 = transforms                    # (H,3,4)

                M = rest_pts.shape[0]
                ones = torch.ones((M, 1), device=rest_pts.device, dtype=rest_pts.dtype)
                hom = torch.cat([rest_pts, ones], dim=1)           # (M,4)

                # Apply each handle transform delta: (M,4) x (H,4,3) -> (M,H,3)
                # Note: transforms_3x4.transpose(1,2) is (H,4,3)
                deltas = torch.einsum('mf,hfo->mho', hom, transforms_3x4.transpose(1, 2))  # (M,H,3)

                # Blend deltas by weights
                disp = (deltas * weights.unsqueeze(-1)).sum(dim=1)  # (M,3)

                # If transforms are *relative deltas* (T - I), then rest + disp is correct
                return rest_pts + disp

            # Register grasp boundary once (pinned_x updated during rollout)
            det_scene.set_object_boundary_condition(
                det_obj_idx,
                name="grasp",
                fcn=_grasp_fcn,
                bdry_penalty=float(sim2real_grasp_penalty),
                pinned_x=_targets_with_rot(0) if (sim2real_use_rotation and tool_R is not None) else _targets_translation(0),
            )

            # -----------------------------
            # 2) Now build anchor constraint and exclude grasp ids
            # -----------------------------
            anchor_mask_cpu = None
            anchor_target = None

            if sim2real_anchor_enable:
                if sim2real_anchor_ids is None:
                    eps_ratio = float(sim2real_anchor_eps_ratio)
                    ...
                    anchor_mask_gpu = torch.zeros((M,), device=device, dtype=torch.bool)
                    anchor_ids = sim2real_anchor_ids.to(device=device).long()
                    anchor_ids = anchor_ids[(anchor_ids >= 0) & (anchor_ids < M)]
                    anchor_mask_gpu[anchor_ids] = True

                # Exclude grasp points (avoid conflicting constraints)
                anchor_mask_gpu[grasp_ids] = False

                # CPU boolean mask required by set_object_boundary_condition
                ...
                anchor_target = internal_rest[anchor_ids_cpu.to(device=device)].clone()

                def anchor_fcn(_deformed_pts):
                ...
                det_scene.set_object_boundary_condition(
                    ...
                    name="anchor",
                    fcn=anchor_fcn,
                    bdry_penalty=float(sim2real_anchor_penalty),
            ...
            def _rollout_fn(current_model: torch.nn.Module, t_idx: int) -> torch.Tensor:
                # Safety caps
                t_idx = int(t_idx)
                t_idx = max(0, min(t_idx, tool_t.shape[0] - 1))
                if t_idx > int(sim2real_max_steps_per_call):
                    t_idx = int(sim2real_max_steps_per_call)

                # Reset scene state (z, zdot, etc.) to rest
                det_scene.reset_scene()

                # Re-apply static anchor pin at t=0 for consistent constraints
                if sim2real_anchor_enable and (anchor_target is not None):
                    det_scene.update_object_boundary_pinned_x("anchor", anchor_target)

                # Re-apply boundary pin at t=0
                det_scene.update_object_boundary_pinned_x(
                    "grasp",
                    _targets_with_rot(0) if (sim2real_use_rotation and tool_R is not None) else _targets_translation(0),
                )

                # Rollout to t_idx (quasi-static: one Newton solve per frame)
                with torch.no_grad():
                    for s in range(1, t_idx + 1):
                        det_scene.update_object_boundary_pinned_x(
                            "grasp",
                            _targets_with_rot(s) if (sim2real_use_rotation and tool_R is not None) else _targets_translation(s),
                        )
                        det_scene.run_sim_step()

                    # Read off the solved handle transforms (relative 4x4) and convert to 3x4 deltas
                    # Here actually only keep the last transform after several steps
                    tfms4 = det_scene.get_object_transforms(det_obj_idx)  # (H,4,4) relative
                    tfms3 = tfms4[:, :3, :].detach().contiguous()

                # Differentiable forward deformation with CURRENT weights, but FIXED Z_t (tfms3)
                W = current_model(internal_rest)  # (M,H)

                pred = _lbs_deform_points(internal_rest, W, tfms3)
                return pred

            sim2real_rollout_fn = _rollout_fn
        ...
        if log_dir is None:
            log_dir = os.path.join(os.getcwd(), "simplicits_train_logs")
        os.makedirs(log_dir, exist_ok=True)

        # -----------------------------
        # Checkpointing / resume
        # -----------------------------
        ckpt_dir = training_ckpt_dir if training_ckpt_dir is not None else os.path.join(log_dir, "checkpoints")
        ...
        def _list_ckpts():
            try:
                ...
                def _step(p):
                    ...
                    m = re.search(r"ckpt_step_(\d+)\.pt", bn)
        ...
        def _save_ckpt(step: int, best: bool = False):
            try:
                ckpt = {
                    "step": int(step),
                ...
                path = os.path.join(ckpt_dir, f"ckpt_step_{int(step):07d}.pt")
                torch.save(ckpt, path)
                if best:
                    torch.save(ckpt, os.path.join(ckpt_dir, "ckpt_best.pt"))
                # keep only last K
                if training_keep_last is not None and int(training_keep_last) > 0:
                    paths = _list_ckpts()
                    if len(paths) > int(training_keep_last):
                        for p in paths[:-int(training_keep_last)]:
                            try:
                                os.remove(p)
                            except Exception:
                                pass
        ...
        # Auto-resume convenience
        if (training_resume_from is None) and bool(training_resume):
            training_resume_from = "latest"

        start_step = 0
        best_total = float("inf")

        # Resolve resume checkpoint
        if training_resume_from is not None:
            resume_path = None
            if isinstance(training_resume_from, str) and training_resume_from.lower() in ["latest", "last"]:
                paths = _list_ckpts()
                resume_path = paths[-1] if len(paths) else None
            else:
                resume_path = training_resume_from
            ...
            if resume_path is not None and os.path.exists(resume_path):
                try:
                    ...
                    try:
                        logger.info(f"Resumed from checkpoint: {resume_path} (start_step={start_step})")
                    except Exception:
                        pass
                except Exception as e:
                    try:
        ...
        if log_csv is None:
            log_csv = os.path.join(log_dir, "train_losses.csv")

        # Write config once (helps reproducibility / reporting)
        if training_save_config:
            cfg = dict(
                num_handles=int(num_handles),
                num_samples=int(num_samples),
                model_layers=int(model_layers),
                training_batch_size=int(training_batch_size),
                training_num_steps=int(training_num_steps),
                training_lr_start=float(training_lr_start),
                training_lr_end=float(training_lr_end),
                training_le_coeff=float(training_le_coeff),
                training_lo_coeff=float(training_lo_coeff),
                training_ls_coeff=float(training_ls_coeff),
                normalize_for_training=bool(normalize_for_training),
                # sim2real knobs (may be None)
                sim2real_enabled=bool((training_ls_coeff is not None) and (training_ls_coeff > 0.0) and (sim2real_rollout_fn is not None)),
                sim2real_train_range=sim2real_train_range,
                sim2real_num_pts_sim=int(sim2real_num_pts_sim),
                sim2real_num_pts_real=int(sim2real_num_pts_real),
                sim2real_scene_num_qp=int(sim2real_scene_num_qp),
                sim2real_scene_timestep=float(sim2real_scene_timestep),
                sim2real_scene_newton_steps=int(sim2real_scene_newton_steps),
                sim2real_max_steps_per_call=int(sim2real_max_steps_per_call),
            )
            try:
        ...
        if log_csv is not None and (not os.path.exists(log_csv)):
            try:
                with open(log_csv, "w", newline="") as f:
                    w = csv.writer(f)
                    w.writerow(["step", "lr", "le", "lo", "ls", "total", "grad_norm", "elapsed_s"])
            except Exception:
                pass


        # Optional eval CSV (no-grad periodic evaluation; useful for notebooks/reports)
        eval_csv = None
        if eval_every is not None and int(eval_every) > 0:
            eval_csv = os.path.join(log_dir, "eval_losses.csv")
            if not os.path.exists(eval_csv):
                try:
                    with open(eval_csv, "w", newline="") as f:
                        w = csv.writer(f)
                        w.writerow(["step", "eval_le", "eval_lo", "eval_ls", "eval_total", "elapsed_s"])
                except Exception:
                    eval_csv = None

        pbar = tqdm(range(start_step, training_num_steps), initial=start_step, total=training_num_steps, disable=not bool(training_progress))
        for i in pbar:
            optimizer.zero_grad(set_to_none=True)

            # Compute Sim2Real loss only occasionally (it is the expensive part).
            # This typically gives a ~sim2real_every× speedup with similar convergence behavior.
            do_sim2real = (training_ls_coeff is not None) and (training_ls_coeff > 0.0) and (sim2real_rollout_fn is not None) \
                          and (i >= int(sim2real_warmup_steps)) and ((i % int(sim2real_every)) == 0)
            ls_coeff_eff = float(training_ls_coeff) if do_sim2real else 0.0

            le, lo, ls = compute_losses(
                model,
                training_pts,
                training_yms,
                training_prs,
                ...
                ls_coeff=ls_coeff_eff,
                sim2real_real_frames=sim2real_real_frames,
                sim2real_train_range=sim2real_train_range,
                sim2real_num_pts_sim=sim2real_num_pts_sim,
                sim2real_num_pts_real=sim2real_num_pts_real,
                sim2real_rollout_fn=sim2real_rollout_fn,
            )

            total = le + lo + ls
            total.backward()

            # Gradient norm is useful for diagnosing instability
            try:
                grad_norm = float(torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=float("inf")).detach().cpu())
            except Exception:
                grad_norm = float("nan")

            optimizer.step()

            # Update learning rate (linear schedule)
            lr = training_lr_start + float(i / training_num_steps) * (training_lr_end - training_lr_start)
            for grp in optimizer.param_groups:
                grp["lr"] = lr

            if (i % training_log_every == 0) or (i == training_num_steps - 1):
                elapsed = time.time() - _t0
                stat = dict(
                    step=int(i),
                    lr=float(lr),
                    le=float(le.detach().cpu()),
                    lo=float(lo.detach().cpu()),
                ...
                # terminal / tqdm postfix
                try:
                    pbar.set_postfix({"le": stat["le"], "lo": stat["lo"], "ls": stat["ls"], "tot": stat["total"]})
                except Exception:
                    pass

                # checkpoints
                try:
                    total_val = float(stat["total"])
                    if total_val < best_total:
                        best_total = total_val
                        _save_ckpt(i, best=True)

                    if training_ckpt_every is not None:
                        if (i % int(training_ckpt_every) == 0) or (i == training_num_steps - 1):
                            _save_ckpt(i, best=False)
                except Exception:
                    pass
                ...
                try:
                    logger.info(
                        f"Training step: {i}, le: {stat['le']}, lo: {stat['lo']}, ls: {stat['ls']}, total: {stat['total']}, grad_norm: {stat['grad_norm']}"
                    )
                except Exception:
                    pass
                ...
                if log_csv is not None:
                    try:
                        with open(log_csv, "a", newline="") as f:
                            w = csv.writer(f)
                            w.writerow([stat["step"], stat["lr"], stat["le"], stat["lo"], stat["ls"], stat["total"], stat["grad_norm"], stat["elapsed_s"]])
                    except Exception:
                        pass

                # user callback (e.g., notebook live plotting)
                if training_callback is not None:
                    try:
                        training_callback(stat)
                    except Exception:
                        pass
            # Periodic evaluation (no-grad). This can be slower if Sim2Real is enabled.
            if eval_every is not None and int(eval_every) > 0 and ((i % int(eval_every)) == 0 or (i == training_num_steps - 1)):
                try:
                    with torch.no_grad():
                        le_e, lo_e, ls_e = compute_losses(
                            model,
                            training_pts,
                            training_yms,
                            training_prs,
                            ...
                            le_coeff=training_le_coeff,
                            lo_coeff=training_lo_coeff,
                            batch_size=max(1, int(training_batch_size)),
                            num_handles=num_handles,
                            appx_vol=training_appx_vol,
                            num_samples=max(64, int(num_samples) // 4),
                            ls_coeff=float(training_ls_coeff),
                            sim2real_real_frames=sim2real_real_frames,
                            sim2real_train_range=sim2real_train_range,
                            sim2real_num_pts_sim=min(int(sim2real_num_pts_sim), 512),
                            sim2real_num_pts_real=min(int(sim2real_num_pts_real), 512),
                            sim2real_rollout_fn=sim2real_rollout_fn,
                        )
                        tot_e = le_e + lo_e + ls_e
                        if eval_csv is not None:
                            elapsed = time.time() - _t0
                            with open(eval_csv, "a", newline="") as f:
                                w = csv.writer(f)
                                w.writerow([int(i), float(le_e.cpu()), float(lo_e.cpu()), float(ls_e.cpu()), float(tot_e.cpu()), float(elapsed)])
                except Exception:
                    pass
        ...
        if training_return_logs:
            # Return logs to caller (e.g., notebook) without breaking default API
            trained_logs = logs
        else:
            trained_logs = None
        ...
        obj = SimplicitsObject(pts, yms, prs, rhos, appx_vol, skinning_weight_function)
        try:
            obj.training_logs = trained_logs
            obj.training_log_dir = log_dir
            obj.training_log_csv = log_csv
        except Exception:
            pass
        if training_return_logs:
            return obj, trained_logs
        return obj
...
class SimplicitsScene:
    ...
    def set_object_boundary_condition(self, obj_idx, name, fcn, bdry_penalty=10000.0, pinned_x=None):
        ...
        # cache indices so we can update pinned_x over time
        if not hasattr(self, "_boundary_cache"):
            self._boundary_cache = {}
        self._boundary_cache[name] = {
            "indices": wp.from_torch(global_bdry_indx, dtype=wp.int32),
            "obj_idx": obj_idx,
        }

    def update_object_boundary_pinned_x(self, name: str, pinned_x: torch.Tensor):
        """Update the target positions for an existing boundary condition.

        This is needed for *moving* boundaries (e.g., tool-driven grasp where the
        grasp patch follows a tool trajectory).

        Args:
            name: boundary name used in set_object_boundary_condition
            pinned_x: (K,3) tensor of new target positions for the same pinned indices
        """
        if not hasattr(self, "_boundary_cache") or name not in self._boundary_cache:
            raise KeyError(f"Boundary '{name}' not found. Call set_object_boundary_condition first.")
        if not self._ready_for_forces:
            self._get_scene_ready_for_forces()
        boundary_struct = self.force_dict["pt_wise"][name]["object"]
        wp_indices = self._boundary_cache[name]["indices"]
        boundary_struct.set_pinned(indices=wp_indices, pinned_x=wp.from_torch(pinned_x, dtype=wp.vec3))
