...
def _chamfer_l2(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    a: (Na,3), b: (Nb,3)
    """
    if a.shape[0] * b.shape[0] > 2_000_000:
        return _chamfer_chunked(a, b, chunk=1024)
    d = torch.cdist(a, b, p=2) ** 2
    return d.min(dim=1).values.mean() + d.min(dim=0).values.mean()


def _chamfer_chunked(x: torch.Tensor, y: torch.Tensor, chunk: int = 1024) -> torch.Tensor:
    """Memory-friendly Chamfer (squared L2). Returns mean(min_x_to_y)+mean(min_y_to_x)."""
    # x: (Nx,3), y: (Ny,3)
    # Compute min distances from x->y in chunks over x
    mins_x = []
    for i in range(0, x.shape[0], chunk):
        d = torch.cdist(x[i:i+chunk], y)  # (c,Ny)
        mins_x.append((d * d).min(dim=1).values)
    mins_x = torch.cat(mins_x, dim=0).mean()

    mins_y = []
    for j in range(0, y.shape[0], chunk):
        d = torch.cdist(y[j:j+chunk], x)  # (c,Nx)
        mins_y.append((d * d).min(dim=1).values)
    mins_y = torch.cat(mins_y, dim=0).mean()
