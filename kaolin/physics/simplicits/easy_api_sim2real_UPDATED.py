...
class SimplicitsObject:
    ...
    def create_trained(pts, yms, prs, rhos, appx_vol,
        ...
        if (training_ls_coeff is not None) and (training_ls_coeff > 0.0) and (sim2real_real_frames is not None) and (sim2real_train_range is not None) and (sim2real_tool_t is not None):
            ...
            det_obj_idx = det_scene.add_object(det_obj, num_qp=int(sim2real_scene_num_qp)) # scene object 
            det_scene.set_scene_gravity(acc_gravity=torch.tensor([0.0, 0.0, 0.0], device=device, dtype=pts.dtype)) # scene force
            det_scene.reset_scene()

            internal_rest = det_scene.get_object_deformed_pts(det_obj_idx)  # (M,3) qp points in rest (apply LBS with zero transforms to the original pts)
